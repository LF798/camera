...
#define APX_K2_EVS_ALGO_WIDTH       (384)
#define APX_K2_EVS_ALGO_HEIGHT      (320)
...
typedef struct _rkMpiVICtx {
    ...
    alp_box_api_t alpPersonBox[ALP_MAX_OUTPUT_NUM];
    alp_rect_api_t alpMotionBox[ALP_MAX_OUTPUT_NUM];
    RK_U32 alpPersonNum;
    RK_U32 alpMotionNum;
} TEST_VI_CTX_S;
...
TEST_VI_CTX_S * ApxCreateVi(uint32_t DevId, uint32_t ChId, uint32_t W, uint32_t H, uint32_t Depth, char* pEntName, PIXEL_FORMAT_E PixelFormat)
{
    ...
    pTempCtx = reinterpret_cast<TEST_VI_CTX_S *>(malloc(sizeof(TEST_VI_CTX_S)));
...
{ 
    ...
    while (!bquit) {
        ...
        input_data.virt_addr = pVirAddr;

        for(RK_U32 i = 0; i< pEvsCtx->alpPersonNum; i++) {
            pTempOutput = &pEvsCtx->alpPersonBox[i];
            pTempOutput->x1 *= 1.25;
            pTempOutput->y1 *= 1.25;
            pTempOutput->x2 *= 1.25;
            pTempOutput->y2 *= 1.25;
            draw_rectangle(&input_data, pEvsCtx->alpPersonBox[i], COLOR_RED, 2);
            printf("show:\tx1=%d, x2=%d, y1=%d, y2=%d\n", pEvsCtx->alpPersonBox[i].x1, pEvsCtx->alpPersonBox[i].x2, pEvsCtx->alpPersonBox[i].y1, pEvsCtx->alpPersonBox[i].y2);
        }

        for(RK_U32 i = 0; i< pEvsCtx->alpMotionNum; i++) {
            pTempMotion = &pEvsCtx->alpMotionBox[i];
            pTempMotion->x1 *= 1.25;
            pTempMotion->y1 *= 1.25;
            pTempMotion->x2 *= 1.25;
            pTempMotion->y2 *= 1.25;
            draw_rectangle(&input_data, pEvsCtx->alpMotionBox[i], COLOR_GREEN, 2);
            printf("show:\tx1=%d, x2=%d, y1=%d, y2=%d\n", pEvsCtx->alpMotionBox[i].x1, pEvsCtx->alpMotionBox[i].x2, pEvsCtx->alpMotionBox[i].y1, pEvsCtx->alpMotionBox[i].y2);
        }
...
int main(int argc, const char **argv) {
    ...
    ApxCreateVo(s32VoLayer, s32VoDev, 0, 192, 236, APX_IMG_SHOW_WIDTH*2, APX_IMG_SHOW_HEIGHT);
...
// --------------- PVS Bind to VO ---------------------------
    ...
    Init_Alp_Detector_Api(handle, "/root/", RK3588_DVS_384x320_INT8);
    ...
    while (!bquit) {
        s32Ret = RK_MPI_VPSS_GetChnFrame(APX_EVS_VPSS_GROUP, APX_EVS_VPSS_SHOW_CHANNEL, &stViFrame, 1000);
        if (s32Ret != RK_SUCCESS) {
            RK_LOGE(" %d RK_MPI_VPSS_GetChnFrame error with code 0x%x\n", __LINE__, s32Ret);
            continue;
        }
        
        uint8_t *pEvsAlgoData = (uint8_t *)RK_MPI_MB_Handle2VirAddr(stViFrame.stVFrame.pMbBlk);
        // memcpy(pEvsAlgoData, imgData, sizeof(uint8_t)*APX_K2_EVS_ALGO_WIDTH*APX_K2_EVS_ALGO_HEIGHT);
        input_data.virt_addr = pEvsAlgoData;

        //RK_LOGE("Evs Algo len = %d", RK_MPI_MB_GetLength(stViFrame.stVFrame.pMbBlk));

        runSpatialDenoiseSimple(pEvsAlgoData, APX_IMG_SHOW_HEIGHT, APX_IMG_SHOW_WIDTH, 127, 2);
        
        Run_Alp_Motion_Api(motion_handle, &input_data, &motion_box, &motion_num);
        if(0 < motion_num) {
            MdStatus++;
            if(3 < MdStatus) {
                MdStatus = 0;
                EvsShowEnable = 1;
                startTimer(&EvsShowTimer, 5);
            }
        } else {
            MdStatus = 0;
        }
        ...
        if(EvsShowEnable) {
            s32Ret = RK_MPI_VPSS_GetChnFrame(APX_EVS_VPSS_GROUP, APX_EVS_VPSS_ALGO_CHANNEL, &stViFramePerson, 1000);
            uint8_t *pEvsPersonData = (uint8_t *)RK_MPI_MB_Handle2VirAddr(stViFramePerson.stVFrame.pMbBlk);
            input_person_data.virt_addr = pEvsPersonData;
            Run_Alp_Detector_Api(handle, &input_person_data, &person_box, &person_num);
            if(0 < person_num) {
                for (size_t i = 0; i < person_num; i++) {
                    if (person_box[i].score > 0.65) {
                        PersonStatus++;
                        break;
                    }
                }
            ...
            s32Ret = RK_MPI_VPSS_ReleaseChnFrame(APX_EVS_VPSS_GROUP, APX_EVS_VPSS_ALGO_CHANNEL, &stViFramePerson);
            if (s32Ret != RK_SUCCESS) {
                RK_LOGE("RK_MPI_VI_ReleaseChnFrame fail, code:0x%x", s32Ret);
            }
        ...
        for (size_t i = 0; i < person_num; i++)
        {
            ...
            person_box[i].x2 *= 2;
            person_box[i].y2 *= 1.9;
            float score = person_box[i].score;
            // printf("detect:\tx1=%d, x2=%d, y1=%d, y2=%d, score=%f\n", person_box[i].x1, person_box[i].x2, person_box[i].y1, person_box[i].y2, person_box[i].score);
            if (person_box[i].score > 0.65)
                draw_rectangle(&input_data, person_box[i], COLOR_RED, 4);
        }

        //for show
        // memset(pEvsCtx->alpPersonBox, 0, sizeof(alp_box_api_t)*ALP_MAX_OUTPUT_NUM);
        // memcpy(pEvsCtx->alpPersonBox, person_box, sizeof(alp_box_api_t)*person_num);
        // pEvsCtx->alpPersonNum = person_num;

        // for (size_t i = 0; i < motion_num; i++)
        // {
        //     int x1 = motion_box[i].x1;
        //     int y1 = motion_box[i].y1;
        //     int x2 = motion_box[i].x2;
        //     int y2 = motion_box[i].y2;
        //     // printf("motion:\tx1=%d, x2=%d, y1=%d, y2=%d\n", x1, x2, y1, y2);
        //     draw_rectangle(&input_data, motion_box[i], COLOR_YELLOW, 4);
        // }

        //for show
        // memset(pEvsCtx->alpMotionBox, 0, sizeof(alp_rect_api_t)*ALP_MAX_OUTPUT_NUM);
        // memcpy(pEvsCtx->alpMotionBox, motion_box, sizeof(alp_rect_api_t)*motion_num);
        // pEvsCtx->alpMotionNum = motion_num;


        if(0 == EvsShowEnable) {
            memset(pEvsAlgoData, 0, EvsImageY);
            memset(pEvsAlgoData + EvsImageY, 128, EvsImageUV);
        }
    ...
    pthread_join(ShowControlThread, RK_NULL);
