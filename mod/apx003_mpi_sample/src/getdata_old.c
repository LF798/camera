...
#define APX_IMG_SHOW_WIDTH              (1280)
#define APX_IMG_SHOW_HEIGHT             (720)

#define EVS_IMG_SHOW_WIDTH              (1280)
#define EVS_IMG_SHOW_HEIGHT             (720)
...
{ 
    ...
    while (!bquit) {
        ...
        if (sem_timedwait(send_donedvs_sem, &out_time) < 0) {
            printf("====== Timeout Send_DoneDvs_sem for EVS data ======\n");
        }
...
{ 
    ...
    while (!bquit) {
        ...
        if (sem_timedwait(send_done_sem, &out_time) < 0) {
            printf("====== Timeout Send_done_sem for APS data ======\n");
        }
...
int main(int argc, const char **argv) {
    ...
    // ApxCreatePvs(APX_IMG_SHOW_WIDTH+EVS_IMG_SHOW_WIDTH, APX_IMG_SHOW_HEIGHT);
...
// --------------- APS  VPSS  Bind to PVS ---------------------------
    ...
    stApsVpssSrcChn.s32DevId = APX_EVS_VPSS_GROUP;
    stApsVpssSrcChn.s32ChnId = APX_APS_VPSS_RESIZE_CHANNEL;

    stApsPvsDestChn.enModId = RK_ID_VO;
    stApsPvsDestChn.s32DevId = s32VoLayer;
