...
#define APX_APS_VPSS_GROUP_NUM          (2)
...
#define APX_IMG_SHOW_WIDTH              (768)
#define APX_IMG_SHOW_HEIGHT             (608)

#define EVS_IMG_SHOW_WIDTH              (768)
#define EVS_IMG_SHOW_HEIGHT             (608)
...
typedef struct {
    ...
    RK_U32 u32Width;        // 宽度
    RK_U32 u32Height;       // 高度
    RK_U32 u32Reserved[4];  // 预留（用于对齐和未来扩展）
} FRAME_METADATA_S;

// APS共享内存大小：元数据 + YUV数据
#define APS_DATA_LEN_WITH_METADATA  (sizeof(FRAME_METADATA_S) + APX_IMG_SHOW_WIDTH * APX_IMG_SHOW_HEIGHT * 3 / 2)

// EVS共享内存大小：元数据 + YUV数据
#define EVS_DATA_LEN_WITH_METADATA  (sizeof(FRAME_METADATA_S) + EVS_IMG_SHOW_WIDTH * EVS_IMG_SHOW_HEIGHT * 3 / 2)
    ...
    for (uint32_t i = 0; i < evsSubFrameNums; i++) {
        ...
        if ((i+1) % APX_K2_EVS_SUB_FRAME_NUM == 0) {
            ...
            if (MergeCount == 4) {
                ...
                // ✅ 修复：使用 VI 原始帧的硬件时间戳，而不是软件时间戳
                stVideoFrame.stVFrame.u64PTS = pstViFrame->stVFrame.u64PTS;
...
{ 
    ...
    // ✅ 修改：使用包含元数据的共享内存大小
    int DVS_DATA_LEN = EVS_DATA_LEN_WITH_METADATA;
    int DVS_YUV_LEN = EVS_IMG_SHOW_WIDTH * EVS_IMG_SHOW_HEIGHT * 3 / 2;
    ...
    RK_LOGI("EVS shared memory initialized: size=%d (metadata=%zu + yuv=%d)", 
            DVS_DATA_LEN, sizeof(FRAME_METADATA_S), DVS_YUV_LEN);
    ...
    while (!bquit) {
        ...
        metadata.u64PTS = stViFrame.stVFrame.u64PTS;
        metadata.u32FrameSeq = (RK_U32)stViFrame.stVFrame.u64PrivateData;
        metadata.u32Width = EVS_IMG_SHOW_WIDTH;
        metadata.u32Height = EVS_IMG_SHOW_HEIGHT;
        memset(metadata.u32Reserved, 0, sizeof(metadata.u32Reserved));
        
        // ✅ 写入共享内存：先元数据，后YUV
        PsrcEvsYuv = (uint8_t*)RK_MPI_MB_Handle2VirAddr(stViFrame.stVFrame.pMbBlk);
        memcpy(papsdata, &metadata, sizeof(FRAME_METADATA_S));
        memcpy(papsdata + sizeof(FRAME_METADATA_S), PsrcEvsYuv, DVS_YUV_LEN);
...
RK_S32 ApxEvsRawDataPullCreate()
{
    ...
    pEvsCtx->stVpssCfg.stVpssChnAttr[APX_EVS_VPSS_SHOW_CHANNEL].u32Width = EVS_IMG_SHOW_WIDTH;
    pEvsCtx->stVpssCfg.stVpssChnAttr[APX_EVS_VPSS_SHOW_CHANNEL].u32Height = EVS_IMG_SHOW_HEIGHT;
    ...
    // 启用 EVS 共享内存线程（供 evs_rtsp_server_v2 使用）
...
{ 
    ...
    // ✅ 修改：使用包含元数据的共享内存大小
    int APS_DATA_LEN = APS_DATA_LEN_WITH_METADATA;
    int APS_YUV_LEN = APX_IMG_SHOW_WIDTH * APX_IMG_SHOW_HEIGHT * 3 / 2;
    ...
    RK_LOGI("APS shared memory initialized: size=%d (metadata=%zu + yuv=%d)", 
            APS_DATA_LEN, sizeof(FRAME_METADATA_S), APS_YUV_LEN);
    ...
    while (!bquit) {
        ...
        metadata.u64PTS = stViFrame.stVFrame.u64PTS;
        metadata.u32FrameSeq = (RK_U32)stViFrame.stVFrame.u64PrivateData;
        metadata.u32Width = APX_IMG_SHOW_WIDTH;
        metadata.u32Height = APX_IMG_SHOW_HEIGHT;
        memset(metadata.u32Reserved, 0, sizeof(metadata.u32Reserved));
        
        // ✅ 写入共享内存：先元数据，后YUV
        PsrcEvsYuv = (uint8_t*)RK_MPI_MB_Handle2VirAddr(stViFrame.stVFrame.pMbBlk);
        memcpy(papsdata, &metadata, sizeof(FRAME_METADATA_S));
        memcpy(papsdata + sizeof(FRAME_METADATA_S), PsrcEvsYuv, APS_YUV_LEN);
...
RK_S32 ApxApsYuvDataPullCreate()
{
    ...
    pApsCtx->stVpssCfg.stVpssChnAttr[APX_APS_VPSS_SHOW_CHANNEL].enChnMode = VPSS_CHN_MODE_USER;
    pApsCtx->stVpssCfg.stVpssChnAttr[APX_APS_VPSS_SHOW_CHANNEL].enDynamicRange = DYNAMIC_RANGE_SDR8;
    pApsCtx->stVpssCfg.stVpssChnAttr[APX_APS_VPSS_SHOW_CHANNEL].enPixelFormat = RK_FMT_YUV420SP;
    pApsCtx->stVpssCfg.stVpssChnAttr[APX_APS_VPSS_SHOW_CHANNEL].stFrameRate.s32SrcFrameRate = -1;
    pApsCtx->stVpssCfg.stVpssChnAttr[APX_APS_VPSS_SHOW_CHANNEL].stFrameRate.s32DstFrameRate = -1;
    pApsCtx->stVpssCfg.stVpssChnAttr[APX_APS_VPSS_SHOW_CHANNEL].u32Width = APX_APS_SRC_WIDTH;
    pApsCtx->stVpssCfg.stVpssChnAttr[APX_APS_VPSS_SHOW_CHANNEL].u32Height = APX_APS_SRC_HEIGHT;
    pApsCtx->stVpssCfg.stVpssChnAttr[APX_APS_VPSS_SHOW_CHANNEL].enCompressMode = COMPRESS_MODE_NONE;
    pApsCtx->stVpssCfg.stVpssChnAttr[APX_APS_VPSS_SHOW_CHANNEL].u32FrameBufCnt = 8;
    pApsCtx->stVpssCfg.stVpssChnAttr[APX_APS_VPSS_SHOW_CHANNEL].u32Depth = 5;
    // init vpss
...
int main(int argc, const char **argv) {
    ...
    stApsVpssSrcChn.s32ChnId = APX_APS_VPSS_SHOW_CHANNEL;

    stVoDestChn.enModId = RK_ID_VO;
    stVoDestChn.s32DevId = s32VoLayer;
    stVoDestChn.s32ChnId = 0;
    s32Ret = RK_MPI_SYS_Bind(&stApsVpssSrcChn, &stVoDestChn);
    if (s32Ret != RK_SUCCESS) {
    ...
    pthread_join(pEvsCtx->ImgShowThread, RK_NULL);  // 等待 EVS 共享内存线程
    pthread_join(pApsCtx->ImgShowThread, RK_NULL);
    RK_LOGE("---- exit ImgHandleThread!");
    

    RK_MPI_SYS_UnBind(&stApsVpssSrcChn, &stVoDestChn);

    ApxApsYuvDataPullDelete();
    ApxEvsRawDataPullDelete();  // 添加 EVS 清理

    RK_MPI_SYS_Exit();

    RK_SAFE_FREE(pApsCtx);
    RK_SAFE_FREE(pEvsCtx);  // 添加 EVS 上下文释放
