...
static RK_S32 util_check_stride_by_pixel(RK_S32 width, RK_S32 hor_stride, RK_S32 pixel_size) {
    if (hor_stride < width * pixel_size) {
        return 1;
    }
...
static RK_S32 util_check_8_pixel_aligned(RK_S32 hor_stride, RK_S32 pixel_aign, RK_S32 pixel_size,
                                         const char *fmt_name) {
    if (hor_stride != RK_ALIGN(hor_stride, pixel_aign * pixel_size)) {
        return 1;
    }
...
RK_S32 TEST_COMM_FillImage(RK_U8 *buf, RK_U32 width, RK_U32 height,
    ...
    case RK_FMT_BGRA4444 : {
        ...
        if (util_check_stride_by_pixel(width, hor_stride, pix_w)) {
            hor_stride *= pix_w;
        }

        if (util_check_8_pixel_aligned(hor_stride,
                                       8, pix_w, "16bit RGB")) {
