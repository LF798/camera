...
RK_S32 TEST_COMM_DumpCompressFrame2File(VIDEO_FRAME_INFO_S *pstFrame, FILE *fp) {
    RK_S32 s32Ret = RK_SUCCESS;
#if HAVE_API_MPI_VGS
    ...
    u32DstSize = RK_MPI_MB_GetLength(pstFrame->stVFrame.pMbBlk);
    ...
    stTask.stImgOut.stVFrame.pMbBlk  = dstBlk;
    ...
    s32Ret = TEST_COMM_DumpFrame2File(&stTask.stImgOut, fp);
__EXIT:
    if (dstBlk) {
        RK_MPI_MB_ReleaseMB(dstBlk);
    }
...
RK_U64 TEST_COMM_GetTotalOsMemory() {
