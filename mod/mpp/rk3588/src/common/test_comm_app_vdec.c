...
static void* TEST_COMM_APP_VDEC_GetFrameProcCB(void *pArgs) {
    TEST_VDEC_CFG_S *pstCfg = (TEST_VDEC_CFG_S *)(pArgs);
...
static RK_S32 TEST_COMM_APP_VDEC_StartBySend(TEST_VDEC_CFG_S *pstCfg) {
    ...
    pstStreamInfo->bUseSeqAsPts = pstCfg->bUseSeqAsPts;
...
RK_S32 TEST_COMM_APP_VDEC_StartProcWithMultiDstChn(TEST_VDEC_CFG_S *pstCfg, MPP_CHN_S *pstDestChn, RK_U32 u32Chns) {
    RK_S32 s32Ret = RK_SUCCESS;
    MPP_CHN_S stSrcChn;
    ...
    for (RK_U32 i = 0; i < u32Chns; i++) {
        s32Ret = RK_MPI_SYS_Bind(&stSrcChn, &pstDestChn[i]);
        if (s32Ret != RK_SUCCESS) {
            RK_LOGE("failed to bind with %#x", s32Ret);
            return s32Ret;
        }
...
RK_S32 TEST_COMM_APP_VDEC_StopProcWithMultiDstChn(TEST_VDEC_CFG_S *pstCfg, MPP_CHN_S *pstDestChn, RK_U32 u32Chns) {
    RK_S32 s32Ret = RK_SUCCESS;
    MPP_CHN_S stSrcChn;
    ...
    for (RK_U32 i = 0; i < u32Chns; i++) {
        s32Ret = RK_MPI_SYS_UnBind(&stSrcChn, &pstDestChn[i]);
        if (s32Ret != RK_SUCCESS) {
            RK_LOGE("failed to unbind with %#x", s32Ret);
            return s32Ret;
        }
