...
static void* TEST_VDEC_SendStreamProc(void *pArgs) {
    ...
    while (pstThreadInfo->bThreadStart) {
        ...
        s32Ret = TEST_COMM_FFmParserRead(pstStreamInfo, &stStreamData);
        if (s32Ret != RK_SUCCESS) {
            continue;
        }
        ...
        if (stStreamData.bEndOfStream) {
            if (s32LoopCnt == -1 || --s32LoopCnt > 0) {  // -1: Infinite loop
                ...
                // TEST_COMM_FFmParserSeek(pstStreamInfo, 0, 2 /* AVSEEK_FLAG_BYTE */);
...
__RETRY:
        ...
        RK_MPI_SYS_Free(stStream.pMbBlk);
        if (stStreamData.bEndOfStream) {
            RK_LOGE("reach eos, pts(%lld)", stStream.u64PTS);
            break;
        }
        if (pstStreamInfo->bUseSeqAsPts)
            stStream.u64PTS += 1;
