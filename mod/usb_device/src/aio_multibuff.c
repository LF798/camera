...
static const struct { //定义USB描述符结构 这些结构体包含了USB设备需要向主机报告的所有信息，包括设备类型、接口描述、端点描述等
...
} __attribute__ ((__packed__)) descriptors = {	//定义USB描述符结构实例
	.header = {
...
int main(int argc, char *argv[])
{
	...
	// }
    if (argc != 3) {
        printf("Usage: %s <ffs_directory> <data_source>\n", argv[0]);
        return 1;
    }
	...
	if (write(ep0, &descriptors, sizeof(descriptors)) < 0) {// 写入描述符和字符串
   ...
   //debug 
    ...
    // 获取数据源文件大小
    fseek(data_source, 0, SEEK_END);
    size_t file_size = ftell(data_source);
	...
	while (total_bytes_read < file_size) {
		 ...
		 */
        // 准备数据传输
        for (unsigned i = 0; i < sizeof(iobuf) / sizeof(*iobuf); ++i) {
            if (iobuf[i].requested) continue;

            // 从数据源读取数据并填充到缓冲区
            size_t bytes_to_read = (file_size - total_bytes_read < iobuf[i].len) ?
                                    (file_size - total_bytes_read) : iobuf[i].len;

            size_t bytes_read = fread(iobuf[i].buf[i], 1, bytes_to_read, data_source);
            if (bytes_read <= 0) {
                break; // 到达文件末尾
            }
            total_bytes_read += bytes_read;

			printf("bytes_to_read:%ld  bytes_read:%ld total_bytes_read:%ld \r\n",bytes_to_read,bytes_read,total_bytes_read);

            // 准备写请求
            io_prep_pwrite(iobuf[i].iocb[i], ep1, iobuf[i].buf[i], bytes_read, 0);
            iobuf[i].iocb[i]->u.c.flags |= IOCB_FLAG_RESFD;
            iobuf[i].iocb[i]->u.c.resfd = evfd;

            // 提交请求
            ret = io_submit(ctx, 1, &iobuf[i].iocb[i]);
            if (ret >= 0) {
                iobuf[i].requested++;
                printf("submit: 1 request buf: %d, bytes: %zu\n", i, bytes_read);
            } else {
