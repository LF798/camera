...
int main(int argc, char *argv[]) {
    ...
    //int APS_DATA_LEN = 1632*1224*3/2;
	int APS_DATA_LEN =640*480*3/2;
    /* alloc buffers and requests */
	buf_in = malloc(APS_DATA_LEN);
	buf_out = malloc(APS_DATA_LEN);
	...
	*/
    // 打开共享内存对象
    int shm_fd = shm_open("/apcdatashm", O_CREAT | O_RDWR, 0777);
	ftruncate(shm_fd,APS_DATA_LEN);
    if (shm_fd == -1) {
        perror("shm_open");
        exit(1);
    }
    ...
    // 将共享内存映射到进程地址空间
    char* papsdata = mmap(0, APS_DATA_LEN, PROT_READ, MAP_SHARED, shm_fd, 0);
    ...
    while (1) {
		...
		if (!req_in) { /* if IN transfer not requested*/
            ...
            io_prep_pwrite(iocb_in, ep[0], papsdata, APS_DATA_LEN, 0);
		...
		if (!req_out) { /* if OUT transfer not requested */
			...
			//io_prep_pread(iocb_out, ep[1], buf_out, APS_DATA_LEN, 0);
			io_prep_pwrite(iocb_out, ep[0], papsdata, APS_DATA_LEN, 0);
...
int main(int argc, char *argv[]) {
	...
	*/
    ...
    while (frame_count < 30) {
            ...
            // 将接收到的数据写入文件
            fwrite(papsdata, 1, EVS_DATA_LEN, file);
            fprintf(stdout, "Received frame %d: len:%d, [%02x%02x%02x%02x%02x%02x]\r\n",
                    frame_count + 1, EVS_DATA_LEN, papsdata[0], papsdata[1], papsdata[2], papsdata[3], papsdata[4], papsdata[5]);
            frame_count++;
    }
    fclose(file);
*/
