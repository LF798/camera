...
struct rtsp_session
{
	...
	#else
	char lz4_reserved[sizeof(uint32_t) * 5 + sizeof(int)];
	#endif
	...
	#else
	char evt2_reserved[sizeof(uint32_t) * 6 + sizeof(int)];
	#endif
...
{
	if (!cc || !s) {
		err("__client_connection_bind_session: invalid parameters\n");
		return;
	}
	if (cc->session == NULL) {
		cc->session = s;
		// 确保队列已初始化（虽然应该在__alloc_session中初始化，但为了安全再次检查）
		// TAILQ_INIT 是幂等的，多次调用是安全的
		TAILQ_INIT(&s->connections_qhead);
...
static int rtsp_path_match (const char *main_path, const char *full_path)
{
	...
	// 安全检查
	if (!main_path || !full_path) {
		err("rtsp_path_match: NULL pointer\n");
		return 0;
	}

	len0 = strlen(main_path);
	len1 = strlen(full_path);
	
	if (len0 == 0 || len1 == 0) {
		return 0;
	}
	
	if (len0 >= sizeof(path0) - 2 || len1 >= sizeof(path1) - 2) {
		err("rtsp_path_match: path too long\n");
		return 0;
	}

	strncpy(path0, main_path, sizeof(path0) - 2);
	strncpy(path1, full_path, sizeof(path1) - 2);
	path0[sizeof(path0) - 1] = '\0';
	...
	if (len0 > 0 && path0[len0 - 1] != '/') {
		if (len0 < sizeof(path0) - 1) {
		strcat(path0, "/");
		}
	}
	if (len1 > 0 && path1[len1 - 1] != '/') {
		if (len1 < sizeof(path1) - 1) {
		strcat(path1, "/");
		}
	}
...
int rtsp_set_video (rtsp_session_handle session, int codec_id, const uint8_t *codec_data, int data_len)
{
	...
	case RTSP_CODEC_ID_VIDEO_LZ4:
		// LZ4支持，使用rtsp_set_video_lz4设置
		break;
	case RTSP_CODEC_ID_VIDEO_EVT2:
		// EVT2支持，使用rtsp_set_video_evt2设置
		break;
	default:
...
static int build_simple_sdp (struct rtsp_session *s, const char *ip, const char *uri, char *sdpbuf, int maxlen)
{
	...
	if (!s || !sdpbuf || maxlen <= 0) {
		err("build_simple_sdp: invalid parameters\n");
		return 0;
	}
	...
	if (s->vcodec_id != RTSP_CODEC_ID_NONE) {
		dbg("build_simple_sdp: vcodec_id=%d\n", s->vcodec_id);
		switch (s->vcodec_id) {
		case RTSP_CODEC_ID_VIDEO_H264:
		...
		case RTSP_CODEC_ID_VIDEO_LZ4:
			// 🔥 LZ4 支持
			#ifdef ENABLE_RTSP_LZ4_SUPPORT
			if (s->lz4_frame_info_valid) {
				p += rtsp_build_sdp_media_attr_lz4(VRTP_PT_ID, s->vrtpe.sample_rate,
						&s->lz4_frame_info, p, maxlen - (p - sdpbuf));
			} else {
				// 使用默认值
				rtsp_lz4_frame_info_t default_info = {768, 608, 0, 700416};
				p += rtsp_build_sdp_media_attr_lz4(VRTP_PT_ID, s->vrtpe.sample_rate,
						&default_info, p, maxlen - (p - sdpbuf));
			}
			#else
			err("LZ4 support not compiled in\n");
			#endif
			break;
		case RTSP_CODEC_ID_VIDEO_EVT2:
			// 🔥 EVT2 支持（完全使用默认值，避免访问可能不安全的字段）
			#ifdef ENABLE_RTSP_EVT2_SUPPORT
			{
				// 直接生成SDP，完全避免访问结构体字段
				// 使用固定的默认值，确保安全
				int pt = VRTP_PT_ID;
				int sample_rate = 90000;  // 固定90kHz，避免访问s->vrtpe.sample_rate
				
				// 使用固定的默认值，完全避免访问evt2_stream_info相关字段
				uint32_t max_events = 50000;
				uint32_t width = 768;
				uint32_t height = 608;
				
				// 直接生成SDP字符串
				int remaining = maxlen - (p - sdpbuf);
				if (remaining >= 200) {
					p += sprintf(p, "m=video 0 RTP/AVP %d\r\n", pt);
					remaining = maxlen - (p - sdpbuf);
					if (remaining > 0) {
						p += sprintf(p, "a=rtpmap:%d EVT2/%d\r\n", pt, sample_rate);
						remaining = maxlen - (p - sdpbuf);
						if (remaining > 0) {
							p += sprintf(p, "a=fmtp:%d max_events=%u;width=%u;height=%u\r\n",
								     pt, max_events, width, height);
						}
					}
				} else {
					err("SDP buffer too small for EVT2: remaining=%d\n", remaining);
				}
			}
			#else
			err("EVT2 support not compiled in\n");
			#endif
			break;
		default:
			err("unsupported video codec_id %d\n", s->vcodec_id);
			break;
		}  // ✅ 
...
//	struct rtsp_demo *d = cc->demo;
	...
	// 安全检查
	if (!cc) {
		err("rtsp_authorization_request: cc is NULL\n");
		return 0;  // 如果cc无效，跳过认证
	}
	...
	if (!s) {
		// session未绑定，跳过认证（DESCRIBE时可能还未绑定）
		return 0;
	}
...
//	struct rtsp_demo *d = cc->demo;
	...
	int sdplen = 0;
	uint32_t accept = 0;
	const rtsp_msg_uri_s *puri = NULL;
	char uri[128] = "";

	// 检查参数
	if (!cc || !reqmsg || !resmsg) {
		err("DESCRIBE: invalid parameters\n");
		return -1;
	}
	...
	// 检查session是否已绑定
	if (!s) {
		warn("DESCRIBE: session not bound\n");
		rtsp_msg_set_response(resmsg, 454);
		return 0;
	}
	
	// 检查URI参数
	puri = &reqmsg->hdrs.startline.reqline.uri;
	if (!puri || !puri->ipaddr) {
		warn("DESCRIBE: invalid URI\n");
	...
	//build uri
	if (snprintf(uri, sizeof(uri), "rtsp://%s", puri->ipaddr) < 0) {
		warn("DESCRIBE: failed to build URI\n");
		rtsp_msg_set_response(resmsg, 400);
		return 0;
	}
	if (puri->port != 0) {
		if (snprintf(uri + strlen(uri), sizeof(uri) - strlen(uri), ":%u", puri->port) < 0) {
			warn("DESCRIBE: failed to append port to URI\n");
			rtsp_msg_set_response(resmsg, 400);
			return 0;
		}
	}
	if (s->path) {
		if (strlen(uri) + strlen(s->path) >= sizeof(uri)) {
			warn("DESCRIBE: URI too long\n");
			rtsp_msg_set_response(resmsg, 400);
			return 0;
		}
	strcat(uri, s->path);
	...
	if (sdplen <= 0 || sdplen >= (int)sizeof(sdpbuf)) {
		warn("DESCRIBE: build_simple_sdp failed or buffer overflow: sdplen=%d\n", sdplen);
	...
	if (!resmsg->body.body) {
		warn("DESCRIBE: rtsp_mem_dup failed\n");
		rtsp_msg_set_response(resmsg, 500);
		return 0;
	}
	return 0;
...
static int rtsp_new_rtp_connection (struct rtsp_client_connection *cc, int isaudio, int istcp, int peer_port, int peer_interleaved)
{
	...
	if (!cc) {
		err("rtsp_new_rtp_connection: cc is NULL\n");
		return -1;
	}
...
//	struct rtsp_demo *d = cc->demo;
	...
	if (!cc || !reqmsg || !resmsg) {
		err("SETUP: invalid parameters\n");
		return -1;
	}
	
	if (!s) {
		warn("SETUP: session not bound\n");
		rtsp_msg_set_response(resmsg, 454);
		return 0;
	}
	...
	if (s->vcodec_id != RTSP_CODEC_ID_NONE && (rtsp_path_match(vpath, reqmsg->hdrs.startline.reqline.uri.abspath)
	 || rtsp_path_match(s->path, reqmsg->hdrs.startline.reqline.uri.abspath))) {
		isaudio = 0;
	...
	if (ret < 0) {
		err("SETUP: rtsp_new_rtp_connection failed\n");
		rtsp_msg_set_response(resmsg, 500);
		return 0;
	}
...
static int rtsp_process_request (struct rtsp_client_connection *cc, const rtsp_msg_s *reqmsg, rtsp_msg_s *resmsg)
{
	...
	// 安全检查
	if (!cc) {
		err("rtsp_process_request: cc is NULL\n");
		return -1;
	}
	if (!reqmsg) {
		err("rtsp_process_request: reqmsg is NULL\n");
		return -1;
	}
	if (!resmsg) {
		err("rtsp_process_request: resmsg is NULL\n");
		return -1;
	}
	
	d = cc->demo;
	if (!d) {
		err("rtsp_process_request: cc->demo is NULL\n");
		rtsp_msg_set_response(resmsg, 500);
		return 0;
	}
	
	s = cc->session;
	path = reqmsg->hdrs.startline.reqline.uri.abspath;
	if (!path) {
		err("rtsp_process_request: path is NULL\n");
		rtsp_msg_set_response(resmsg, 400);
		return 0;
	}
	...
	if (s) {
		if (reqmsg->hdrs.startline.reqline.method != RTSP_MSG_METHOD_TEARDOWN &&
			reqmsg->hdrs.startline.reqline.method != RTSP_MSG_METHOD_PLAY &&
			reqmsg->hdrs.startline.reqline.method != RTSP_MSG_METHOD_PAUSE) {
			// 对于其他请求（如 DESCRIBE、SETUP），仍然需要路径匹配
			if (s->path && path && rtsp_path_match(s->path, path) == 0) {
		...
		TAILQ_FOREACH(s, &d->sessions_qhead, demo_entry) {
			if (s && s->path && rtsp_path_match(s->path, path)) {
				break;
			}
		}
		if (NULL == s) {
...
static int rtsp_send_msg (struct rtsp_client_connection *cc, rtsp_msg_s *msg) 
{
	...
	if (cc->reslen > 0) {
		ret = send(cc->sockfd, cc->resbuf + cc->resoff, cc->reslen, MSG_NOSIGNAL);
		if (ret != cc->reslen) {
	...
	ret = send(cc->sockfd, cc->resbuf, size, MSG_NOSIGNAL);
	if (ret != size) {
		if (ret > 0 && ret < size) {
			cc->resoff = ret;
			cc->reslen = size - ret;
...
static int rtsp_send_interlaced_frame (struct rtsp_client_connection *cc, int channel, const uint8_t *data, int len)
{
	...
	*((uint16_t*)&szbuf[2]) = htons(len);
	ret = send(cc->sockfd, (const char*)szbuf, 4, MSG_NOSIGNAL);
	if (ret != 4) {
		if (ret > 0 && ret < 4) {
			cc->resoff = 0;
			cc->reslen = 4 - ret + len;
	...
	ret = send(cc->sockfd, (const char*)data, len, MSG_NOSIGNAL);
	if (ret != len) {
		if (ret > 0 && ret < len) {
			cc->resoff = 0;
			cc->reslen = len - ret;
...
//					s->vcodec_data.h265.vps, s->vcodec_data.h265.sps_len, s->vcodec_data.h265.pps_len);
	...
	// ✅ 更新时间戳统计信息
	if (!s->video_ts_stats.initialized) {
		// 首帧初始化
		s->video_ts_stats.first_pts = ts;
		s->video_ts_stats.last_pts = ts;
		s->video_ts_stats.prev_pts = ts;
		s->video_ts_stats.min_interval_us = UINT64_MAX;
		s->video_ts_stats.max_interval_us = 0;
		s->video_ts_stats.total_interval_us = 0;
		s->video_ts_stats.frame_count = 1;
		s->video_ts_stats.total_bytes = len;
		s->video_ts_stats.initialized = 1;
	} else {
		// 更新统计信息
		uint64_t interval = 0;
		if (ts > s->video_ts_stats.prev_pts) {
			interval = ts - s->video_ts_stats.prev_pts;
			s->video_ts_stats.total_interval_us += interval;
			
			// 更新最小/最大间隔
			if (interval < s->video_ts_stats.min_interval_us) {
				s->video_ts_stats.min_interval_us = interval;
			}
			if (interval > s->video_ts_stats.max_interval_us) {
				s->video_ts_stats.max_interval_us = interval;
			}
		}
		
		s->video_ts_stats.last_pts = ts;
		s->video_ts_stats.prev_pts = ts;
		s->video_ts_stats.frame_count++;
		s->video_ts_stats.total_bytes += len;
	}
...
int rtsp_get_video_ts_info(rtsp_session_handle session, rtsp_ts_info_t *info)
{
	...
	if (!s->video_ts_stats.initialized || s->video_ts_stats.frame_count == 0)
		return -1;  // 未发送任何视频帧
	
	// 填充时间戳信息
	info->first_pts = s->video_ts_stats.first_pts;
	info->last_pts = s->video_ts_stats.last_pts;
	info->min_interval_us = s->video_ts_stats.min_interval_us;
	info->max_interval_us = s->video_ts_stats.max_interval_us;
	info->frame_count = s->video_ts_stats.frame_count;
	info->total_bytes = s->video_ts_stats.total_bytes;
	
	// 计算平均帧间隔
	if (s->video_ts_stats.frame_count > 1) {
		info->avg_interval_us = s->video_ts_stats.total_interval_us / 
		                       (s->video_ts_stats.frame_count - 1);
	} else {
		info->avg_interval_us = 0;
	}
...
int rtsp_reset_video_ts_info(rtsp_session_handle session)
{
	...
	// 清零所有统计信息
	s->video_ts_stats.first_pts = 0;
	s->video_ts_stats.last_pts = 0;
	s->video_ts_stats.prev_pts = 0;
	s->video_ts_stats.min_interval_us = UINT64_MAX;
	s->video_ts_stats.max_interval_us = 0;
	s->video_ts_stats.total_interval_us = 0;
	s->video_ts_stats.frame_count = 0;
	s->video_ts_stats.total_bytes = 0;
	s->video_ts_stats.initialized = 0;
