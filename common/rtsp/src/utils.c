...
int rtsp_build_sdp_media_attr_lz4 (int pt, int sample_rate, const rtsp_lz4_frame_info_t *frame_info, char *sdpbuf, int maxlen)
{
	...
	p += sprintf(p, "m=video 0 RTP/AVP %d\r\n", pt);
	p += sprintf(p, "a=rtpmap:%d LZ4/%d\r\n", pt, sample_rate);
	if (frame_info) {
		p += sprintf(p, "a=fmtp:%d width=%u;height=%u;format=grayscale8;original-size=%u\r\n",
			     pt, frame_info->width, frame_info->height, frame_info->original_size);
	} else {
		p += sprintf(p, "a=fmtp:%d width=768;height=608;format=grayscale8;original-size=467264\r\n", pt);
	}
...
int rtsp_build_sdp_media_attr_evt2 (int pt, int sample_rate, const rtsp_evt2_stream_info_t *stream_info, char *sdpbuf, int maxlen)
{
	...
	// 使用sprintf，与LZ4保持一致（LZ4使用sprintf没有问题）
	p += sprintf(p, "m=video 0 RTP/AVP %d\r\n", pt);
	p += sprintf(p, "a=rtpmap:%d EVT2/%d\r\n", pt, sample_rate);
	if (stream_info) {
		p += sprintf(p, "a=fmtp:%d max_events=%u;width=%u;height=%u\r\n",
			     pt, stream_info->max_events_per_frame, 
			     stream_info->width, stream_info->height);
	} else {
		p += sprintf(p, "a=fmtp:%d max_events=50000;width=768;height=608\r\n", pt);
	}
